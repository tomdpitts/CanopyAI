...
def train_deepforest(
    ...
    _val_transform = A.Compose(
        [A.CenterCrop(height=CROP_SIZE, width=CROP_SIZE, p=1.0), ToTensorV2()],
    ...
    if val_csv:
        ...
        if _val_empty_paths:
            ...
            def _val_dataloader_centered(self_model):
                dl = _orig_vl(self_model)
                dl.dataset.transform = _val_transform
                return DataLoader(dl.dataset, batch_size=dl.batch_size, shuffle=False,
                                  collate_fn=dl.dataset.collate_fn,
                                  num_workers=4, pin_memory=True, persistent_workers=True)
            model.val_dataloader = types.MethodType(_val_dataloader_centered, model)
