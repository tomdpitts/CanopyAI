...
Usage:
    python foxtrot_TCD.py --image_path data/tcd/bin_liang/tcd_tile_WON.tif
    python foxtrot_TCD.py --use_test_data  # Run evaluation on TCD test set
...
# Validation utilities from utils.py (same as infer.py)
from utils import (
    load_tcd_meta_for_tile,
...
def save_results(
    ...
    Args:
        ...
        crs: Coordinate reference system
        transform: Rasterio Affine transform for pixel->world conversion
    """
    ...
    for i, (bbox, score) in enumerate(zip(bboxes, deepforest_scores)):
        ...
        # Transform from pixel to world coordinates if transform is provided
        if transform is not None:
            ...
            polygon = shapely_affine(polygon, coeffs)
...
def parse_args():
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Path to input TIF file (required unless --use_test_data is set)",
    )

    ap.add_argument(
        "--use_test_data",
        action="store_true",
        help="Run evaluation on TCD test data (data/tcd/raw_test/). "
        "Outputs precision, recall, F1 scores.",
    )
...
def masks_to_geodataframe(cache_files, bboxes, scores, crs, transform):
    """
    Convert SAM masks to a GeoDataFrame for validation.
    ...
    Args:
        ...
        scores: List of confidence scores
        crs: Coordinate reference system
        transform: Rasterio transform for pixel->world conversion

    Returns:
        GeoDataFrame with polygon geometries in world coordinates
    """
    from shapely.affinity import affine_transform as shapely_affine
    ...
    for i, (bbox, score) in enumerate(zip(bboxes, scores)):
        ...
        world_polygon = shapely_affine(polygon, coeffs)
    ...
    if not features:
        return gpd.GeoDataFrame(columns=["geometry", "score", "bbox"], crs=crs)
...
def compute_one_to_one_metrics(pred_gdf, gt_gdf, iou_threshold=0.5):
    """
    Compute metrics using 1-to-1 matching (greedy assignment) to strictly
    prevent Recall > 1.0.

    Args:
        pred_gdf: GeoDataFrame of predictions
        gt_gdf: GeoDataFrame of ground truth
        iou_threshold: Minimum IoU to consider a match

    Returns:
        dict: precision, recall, f1, n_tp, n_fp, n_fn
    """
    if len(pred_gdf) == 0:
        return {
            "precision": 0.0,
    ...
    # We'll use a spatial index for efficiency
    sindex = gt_gdf.sindex

    matches = []  # (iou, pred_idx, gt_idx)

    for pred_idx, pred_geom in enumerate(pred_gdf.geometry):
        if not pred_geom.is_valid or pred_geom.is_empty:
            continue

        # Find potential intersections
        possible_matches = list(sindex.query(pred_geom, predicate="intersects"))

        for gt_idx in possible_matches:
            gt_geom = gt_gdf.geometry.iloc[gt_idx]
    ...
    # 2. Sort by IoU descending (Greedy matching)
...
def main_test_data(args):
    """
    Run evaluation on TCD test data and output precision/recall/F1 metrics.
    Uses same evaluation methodology as infer.py.
    """
    ...
    if len(files_to_process) == 0:
        raise FileNotFoundError(
            ...
            "Please ensure test data exists in data/tcd/raw_test/"
        )

    print(f"\n{'=' * 60}")
    print("🧪 TCD Test Data Evaluation: DeepForest + SAM Pipeline")
    print(f"{'=' * 60}")
    print(f"Test tiles found: {len(files_to_process)}")
    ...
    # Load finetuned decoder if provided
    if args.sam_finetuned_decoder:
        print(f"🎯 Loading finetuned decoder from {args.sam_finetuned_decoder}...")
        if not Path(args.sam_finetuned_decoder).exists():
            raise FileNotFoundError(
    ...
    # Accumulators for cohort metrics
    cohort_stats = {
        "n_tp_tree": 0,
        "n_fp_tree": 0,
        "n_fn_tree": 0,
    ...
    for img_path in files_to_process:
        ...
        # Load image
        image, crs, transform, bounds = load_image_from_tif(img_path)
        ...
        if len(all_bboxes) == 0:
            print(f"⚠️  No trees detected in {img_path.name}, skipping.")
            continue
        ...
        valid_bboxes = [all_bboxes[i] for i in keep_indices]
        valid_scores = [all_scores[i] for i in keep_indices]
        print(f"   After NMS: {len(valid_bboxes)} trees")

        # Pass 2: SAM segmentation
        cache_dir = Path(tempfile.mkdtemp(prefix="foxtrot_eval_"))
        cache_files, valid_bboxes, valid_scores = segment_trees_sam(
            image,
            sam_predictor,
            valid_bboxes,
        ...
        if len(valid_bboxes) == 0:
            print(f"⚠️  No valid segmentations in {img_path.name}, skipping.")
            shutil.rmtree(cache_dir, ignore_errors=True)
            continue

        # Save GeoJSON for validation
        ...
        # Skip validation if no GT metadata
        if image_id == "unknown" or image_info is None:
            print(f"⚠️  No ground truth metadata for {img_path.name} — skipping.")
            shutil.rmtree(cache_dir, ignore_errors=True)
            continue

        # ---------------------------------------------------------------------
        # VALIDATION (Strict 1-to-1)
        # ---------------------------------------------------------------------
        # Reuse validation util to parse GT but not metric calc
        _, pred_gdf, gt_gdf, scores_tuple, coco_anns = (
            clean_validate_predictions_vs_tcd_segments(
                pred_geojson_path=output_geojson_path,
        ...
        if gt_gdf is None or len(gt_gdf) == 0:
            print("   Skipping metrics (No GT polygons)")
            shutil.rmtree(cache_dir, ignore_errors=True)
            continue

        # Split GT into trees and canopy
        gt_trees = gt_gdf[gt_gdf["category"] == 2]
        gt_canopy = gt_gdf[gt_gdf["category"] == 1]

        # Compute metrics for TREES
        m_trees = compute_one_to_one_metrics(pred_gdf, gt_trees, iou_threshold=0.5)

        # Compute metrics for CANOPY
        m_canopy = compute_one_to_one_metrics(pred_gdf, gt_canopy, iou_threshold=0.5)

        print("\n   📊 Tile Metrics (1-to-1):")
        print(
            f"     Trees  (IoU>=0.5): F1={m_trees['f1']:.3f} (TP={m_trees['n_tp']}, GT={len(gt_trees)})"
        )
        print(
            f"     Canopy (IoU>=0.5): F1={m_canopy['f1']:.3f} (TP={m_canopy['n_tp']}, GT={len(gt_canopy)})"
        )

        # Accumulate counts
        cohort_stats["n_tp_tree"] += m_trees["n_tp"]
        cohort_stats["n_fp_tree"] += m_trees["n_fp"]
        cohort_stats["n_fn_tree"] += m_trees["n_fn"]
        cohort_stats["n_gt_tree_total"] += len(gt_trees)

        cohort_stats["n_tp_canopy"] += m_canopy["n_tp"]
        cohort_stats["n_fp_canopy"] += m_canopy["n_fp"]
        cohort_stats["n_fn_canopy"] += m_canopy["n_fn"]
        cohort_stats["n_gt_canopy_total"] += len(gt_canopy)
        cohort_stats["n_pred_total"] += len(pred_gdf)
    ...
    # Trees
    t_tp = cohort_stats["n_tp_tree"]
    t_fp = cohort_stats["n_fp_tree"]  # Note: this assumes all non-tree preds are FP
    t_fn = cohort_stats["n_fn_tree"]
    t_prec, t_rec, t_f1 = calc_f1(t_tp, t_fp, t_fn)

    # Canopy
    c_tp = cohort_stats["n_tp_canopy"]
    c_fp = cohort_stats["n_fp_canopy"]
    c_fn = cohort_stats["n_fn_canopy"]
    c_prec, c_rec, c_f1 = calc_f1(c_tp, c_fp, c_fn)

    print(f"\n{'=' * 60}")
    print("📊 COHORT METRICS (All Tiles) - STRICT 1-to-1 MATCHING")
    print(f"{'=' * 60}")
    ...
    print(
        f"  tp={t_tp}, fp={t_fp}, fn={t_fn}, total_gt={cohort_stats['n_gt_tree_total']}"
    )
    ...
    print(
        f"  tp={c_tp}, fp={c_fp}, fn={c_fn}, total_gt={cohort_stats['n_gt_canopy_total']}"
    )

    print(f"\nTotal Predictions: {cohort_stats['n_pred_total']}")
    print(f"{'=' * 60}")
...
if __name__ == "__main__":
    ...
    if args.use_test_data:
        ...
        print("❌ Error: Either --image_path or --use_test_data is required.")
        print("Usage:")
        print("  python foxtrot_TCD.py --image_path <path/to/image.tif>")
        print("  python foxtrot_TCD.py --use_test_data")
        exit(1)
